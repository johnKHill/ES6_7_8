    // Option 1: With the template provided, write a template tag that will validate an SQL statement:

    // Only SELECT or UPDATE statements are used
    // The password table is off limits
    // If there is no order by clause add it and order by asc
    ...
    if((command.indexOf(`SELECT`)) == -1 && (command.indexOf(`UPDATE`)) == -1){
        return (`YOU are only [privilaged to make SELECT and UPDATE.`)
            ...
            return (`You are not priviledged yo alter that table.`)
        } else if(!order){
            order =` ORDER by asc`
        }
