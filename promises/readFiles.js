// CHECK. 1. Get file names (from the files in data.zip) using fs module's readdir. 
// CHECK. 2. Loop through and print off the name of each file in the dir
// CHECK. 3. Using readFile (NOT readFileSync), read the files and in the callback, print the filename. Note: They will not be the same as in the previous step. 
// CHECK. 4. Look at the file contents, count up the total number of the string "gotYa" in each. Print an array that has the count for each file in ascending order. 
//      For a little more work, make it an object with a key of file name, value of count instead. 
// CHECK. 5. As soon as the full contents of any single are read, print them out. Do not wait for any other file to complete.
...
fs.readdir('./data', (err, files) => {
    ...
    // Cast the data from readFile to a String so we can manipulate it into a string
